...
// ── fetch services ───────────────────────────────────────────────────────
$services = $pdo->query("
  SELECT OfferingID, OfferingName, ServiceDescription, MinPrice, MaxPrice, ImagePath
  FROM ServiceOffering
  ORDER BY OfferingID DESC
")->fetchAll(PDO::FETCH_ASSOC);

// ── fetch coupons (now sorted ASC by CouponNumber) ────────────────────────
...
<body class="d-flex">
  ...
  <div class="container-fluid p-4">
    ...
    <section id="appointments-history" class="mb-5">
      ...
      <table class="table table-sm table-bordered mb-4">
        <thead class="table-light">
          <tr>
            <th>Customer</th><th>Service</th><th>Start</th><th>End</th><th>Price</th>
          </tr>
        </thead>
        <tbody>
      ...
      <table class="table table-sm table-bordered mb-4">
        <thead class="table-light">
          <tr><th>Customer</th><th>Service</th><th>Start</th><th>End</th><th>Price</th></tr>
        </thead>
        <tbody>
    ...
    <section id="add-services" class="mb-5">
      ...
      <form id="service-form" action="add_service.php" method="POST" enctype="multipart/form-data" class="row g-3">
        <div class="col-md-3">
        ...
        <div class="col-md-2">
          ...
          <input id="minPrice" name="minPrice" type="number" step="0.01" class="form-control" required>
        </div>
        <div class="col-md-2">
          <label class="form-label">Max Price ($)</label>
          <input id="maxPrice" name="maxPrice" type="number" step="0.01" class="form-control" required>
        </div>
        <div class="col-md-3">
    ...
    <table class="table table-striped align-middle mb-5">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <?php if($s['ImagePath']): ?>
              <img src="<?=htmlspecialchars($s['ImagePath'])?>" class="img-fluid" style="max-height:60px">
            <?php endif; ?>
          </td>
          ...
          <td class="text-end">
            <a href="edit_service.php?id=<?=$s['OfferingID']?>" class="btn btn-sm btn-outline-secondary me-1">Edit</a>
    ...
    <section id="coupon-panel" class="mb-5">
      ...
      <form id="coupon-form" action="add_coupon.php" method="POST" class="row g-3 mb-4">
        <div class="col-md-3">
          ...
          <input id="discountAmt" name="amount" type="number" step="0.01" class="form-control" required>
        </div>
        <div class="col-md-4">
          <label class="form-label">Service Offering</label>
          <select id="couponSvc" name="offering_id" class="form-select" required>
      ...
      <table class="table table-striped align-middle mb-5">
        <thead>
          <tr><th>Coupon #</th><th>Amount</th><th>Offering</th><th></th></tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td class="text-end">
              <a href="edit_coupon.php?id=<?=$c['CouponNumber']?>" class="btn btn-sm btn-outline-secondary me-1">Edit</a>
 ...
 <section id="add-employee" class="mb-5">
  ...
  <script>
    ...
    // ── Validate Add Coupon: discount ≤ selected max ─────────────────────
    ...
    // ── Employee‐modal wiring (unchanged) ────────────────────────────────
    document.querySelectorAll('.view-employee').forEach(btn => {
      btn.addEventListener('click', () => {
        ['Name','Email','Phone','Title','Spec','Jobs'].forEach(f => {
          document.getElementById('emp'+f).textContent = btn.dataset[f.toLowerCase()];
        });
      });
    });
  </script>
